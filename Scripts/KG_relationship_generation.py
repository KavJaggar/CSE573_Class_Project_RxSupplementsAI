...
- ALL PREDICATES MUST BE 2 WORDS OR FEWER - this is a hard requirement
...
def run_query():
    ...
    with open("../CorpusData/natmed_documents.json", "r", encoding="utf-8") as f:
        ...
        for i in range(100):
            try:
                doc = documents[i]
                prompt = MAIN_SYSTEM_PROMPT + doc["herb"] + "-" + doc["section"] + " : " + doc["text"]
                ...
                temp = generate(prompt, url, payload)
                print() #newline
                print() #newline
                ...
                first = True
                try:
                    with open("../KnowledgeGraphData/KGRelationshipsA.json", "r", encoding="utf-8") as f:
                        toins = json.load(f)
                ...
                if first:
                    with open("../KnowledgeGraphData/KGRelationshipsA.json", "w", encoding="utf-8") as f:
                        json.dump(parsed, f, indent=2)
                else:
                    with open("../KnowledgeGraphData/KGRelationshipsA.json", "w", encoding="utf-8") as f:
                        json.dump(toins, f, indent=2)
            except Exception:
                print(f"Failed: {i}, moving to next.")
